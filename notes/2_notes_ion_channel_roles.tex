...
% Bursting: T-Type + i\_h (sag) \parencite{wangMultipleDynamicalModes1994}

% T-Type and h current are largly responsible for subthreshold
% properties and slow oscillations in thalamic cells \parencite{wangMultipleDynamicalModes1994}

% Wang 1994: Bistability due to h-current - bursring vs subthreshold 
% oscillations based on initial step current (0.25 vs -1.2 to -0.47) \parencite{wangMultipleDynamicalModes1994}

% Activation of K currents - response to depolarizing current got smaller \parencite{mccormickModelElectrophysiologicalProperties1992}

% CA1 pyramidal cells experimentally exhibited very slow dynamics (> 1s) that
% may cause rhythmic bursting to change to repetitive spikes \parencite{golombContributionPersistentNa2006}  

% Reducing Ca outside concentration is not the same as blocking Ca currents \parencite{golombContributionPersistentNa2006}

% Ns increased with gCa for small gC values and decreased with gCa fir karge gC values \parencite{golombContributionPersistentNa2006}
...
% burst of high-frequency APs \parencite{vickstromTTypeCalciumChannels2020}.

% Are inactivated at typical resting membrane potentials, can be
% de-inactivated by hyperpolarization - "rebound bursting" \parencite{vickstromTTypeCalciumChannels2020}


% Blocking T-Type channels with antagonist Z944 significantly reduced or abolished
% bursting activity \parencite{vickstromTTypeCalciumChannels2020}

% Imparts possibility to bursting at specific freqeucny \parencite{wangMultipleDynamicalModes1994}

% T Should provide the inward current for the generation of low-threshold
% Ca sppikes \parencite{huguenardSimulationCurrentsInvolved1992}

% Underlines burst firing \parencite{huguenardSimulationCurrentsInvolved1992}
...
% Mediate LVA current in ET cells \parencite{liuMultipleConductancesCooperatively2008}

% "Window T-Type Ca current plays no obvious role in ET cell bursting"
% \parencite{liuMultipleConductancesCooperatively2008}

% "Activation of T-Type Ca channels is required for low-threshold Ca spikes and
% spontaneous bursting" \parencite{liuMultipleConductancesCooperatively2008}

% "Localizing T-channels in the dendrites decreases the excitability of the cell with
% respect to LTS generation" \parencite{destexheDendriticLowthresholdCalcium1998}
...
% I\_T resulted in a large rebound Ca spike after hyperpolarization \parencite{mccormickModelElectrophysiologicalProperties1992}

% LTS: Low threshold Ca spike \parencite{mccormickModelElectrophysiologicalProperties1992}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \textbf{H current}

% (Computational) Research of interest because suggested importance \parencite{destexheModelInwardCurrent1993}
...
% One function is to generate pacemaker activity \parencite{liuMultipleConductancesCooperatively2008}

% Contributes to setting minimal membrane potential (MMP) - makes it more depolarized
% level \parencite{liuMultipleConductancesCooperatively2008}
...
% I\_h resulted in a depolarizing sag in response to hyperpolarization \parencite{mccormickModelElectrophysiologicalProperties1992}

% Apparent afterhyperpolarization after Ca spike \parencite{mccormickModelElectrophysiologicalProperties1992}

% Increases burst frequency \parencite{mccormickModelElectrophysiologicalProperties1992}
...
% Modulating initial components of low-threshold Ca spikes \parencite{huguenardSimulationCurrentsInvolved1992}

% May fascilitate slow repetitive fring and may interact with I\_T during Ca-dpendent
% burst fiing \parencite{huguenardSimulationCurrentsInvolved1992}

% May contribute to both - low-threshold Ca spikes and fast Na spikes \parencite{huguenardNovelTtypeCurrent1992}

% Activation kinetics are probably sufficiently fast to allow contribution
% ti the repolarization of Na-dependent action potentials \parencite{huguenardSimulationCurrentsInvolved1992}

% Addtion of I\_A resulted in a delay in the response of the model cell to a depolarizing
% current pulse \parencite{mccormickModelElectrophysiologicalProperties1992}

% Slowed rate of action potential generation \parencite{mccormickModelElectrophysiologicalProperties1992}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \textbf{I\_K2 - Slowly activating and inkactivating K}

% May affect more later portions of low-threshold Ca spikes
% (later than initial components modulated by I\_A) \parencite{huguenardSimulationCurrentsInvolved1992}

% May control repetitive firing rate \parencite{huguenardSimulationCurrentsInvolved1992}

% LTS: Repolarization \parencite{mccormickModelElectrophysiologicalProperties1992}
...
% and input resistance \parencite{liuMultipleConductancesCooperatively2008}
...
% adaptation \parencite{golombContributionPersistentNa2006}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \textbf{L-Type current}

% Mediate Ca LVA current in PG cells \parencite{liuMultipleConductancesCooperatively2008}
...
% increase the number of spikes per burst" \parencite{liuMultipleConductancesCooperatively2008}
...
% TTX sensitive NaP \parencite{liuMultipleConductancesCooperatively2008}

% Increasing NaP in the model increases number of spikes within a burst
% but is neither necessary nor sufficient for bursting \parencite{golombContributionPersistentNa2006}

% Bursting in low ca outside concentration is driven predominantly by NaP \parencite{golombContributionPersistentNa2006}

% Bursting in truncated neurons bathed in ca free is due to interplay between NaP and I\_M \parencite{golombContributionPersistentNa2006}

% Increasgin g may induce bursting behaviour and increase Ns \parencite{golombContributionPersistentNa2006}

% As g NaP increased the propensity for bursting increased, so that at 0.08 neuron became periodic burster \parencite{golombContributionPersistentNa2006}

% For given Ns, f increased as gNaP or Iapp increased \parencite{golombContributionPersistentNa2006}

% For other parameter regime Ns increased with gNaP and decreaed with gM \parencite{golombContributionPersistentNa2006}
...
% of ET cells. \parencite{liuModelNeuronActivityDependent1998}
...
% currents are sufficiently, although not extremely fast \parencite{golombContributionPersistentNa2006}

% For other parameter regime Ns increased with gNaP and decreaed with gM \parencite{golombContributionPersistentNa2006}

% For given Ns, f decreased as gM increased \parencite{golombContributionPersistentNa2006}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \textbf{Leak Currents}

% Two leak currents K and Na determined in large part the resting membrane potential and
% apparent input resistance of te model neuron \parencite{mccormickModelElectrophysiologicalProperties1992}.

% RMP and resting input resistance \parencite{mccormickModelElectrophysiologicalProperties1992}
